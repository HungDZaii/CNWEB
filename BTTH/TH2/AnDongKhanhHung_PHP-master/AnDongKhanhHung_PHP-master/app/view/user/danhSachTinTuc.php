...
<style>
    ...
    .flower-item {
        ...
        border-radius: 8px;
        width: 100%;
        /* Chiều rộng bằng container */
        max-width: 500px;
        /* Giới hạn chiều rộng từng mục */
...
<body>
    <div class="container">
        <h1>Danh sách tin tức</h1>
        <div class="flower-list">
            <?php foreach ($newList as $index => $value):?>
                <a href="index.php?action=getChiTietTinTuc&id=<?= $value['id'] ?>&name=<?= urlencode($value['name']) ?>&title=<?= urlencode($value['title']) ?>&content=<?= urlencode($value['content']) ?>&image=<?= urlencode($value['image']) ?>">
                <div class="flower-item">
                    <p><?= $value['name'] ?></p>
                    <p><?= $value['title'] ?></p>
                    <p><?= $value['content'] ?></p>
                    <img src="<?= $value['image']?>" alt="<?= $value['name'] ?>" class="flower-image">
                </div>
                </a>
            <?php endforeach; ?>
